# Stage 1: Build Olaf with Zig 0.16.0
...
# Download and extract Zig 0.16.0 for the target architecture
RUN if [ "$TARGETARCH" = "arm64" ]; then \
      ZIG_ARCH="aarch64"; \
    else \
      ZIG_ARCH="x86_64"; \
    fi && \
    wget -q "https://ziglang.org/download/0.16.0/zig-${ZIG_ARCH}-linux-0.16.0.tar.xz" && \
    tar xf zig-${ZIG_ARCH}-linux-0.16.0.tar.xz && \
