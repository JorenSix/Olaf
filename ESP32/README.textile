...
The 'index' or header file can be created on a computer to contain fingerprints extracted from audio. By default it recognizes 'Let It Go' from Frozen. To create this header file for your audio do the following:
...
By default the @src/olaf_fp_ref_mem.h@ is included in the ESP32 code. To test and debug this header file, use the @mem@ version of Olaf on your computer. The ESP32 version is basically the same as the @mem@ version, only the audio comes from a MEMS microphone input and not from a file.

Once your are sure that the @mem@ version of Olaf works as expected and the INMP441 microphone is tested the ESP32 version can be ran using the Arduino IDE.