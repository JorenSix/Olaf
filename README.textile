...
The power of Zig is, however, more obvious when an other target is provided. For a list of platforms, please consult the output of @zig targets@. To build a windows executable run the following commands. Afterwards, pre-built windows executable can be found in @pre-built@. There is further "documentation for running Olaf on Windows":pre-built/README.textile.
...
h3. Cache fingerprints and store cached fingerprints
...
The ESP32 version does not use a key value store but a list of hashes stored in a @.h@ header file. To make this header file do the following:
...
h2. Benchmark, evaluation and testing of Olaf

Every time the source code of Olaf is updated a number of functional checks are done. The first thing this checks is whether Olaf compiles correctly. If it does a small dataset is indexed and some queries are fired. The result of the queries is evaluated for correctness. Also the memory version of Olaf is checked. To run this yourself, with Ruby, ffmpeg and ffprobe installed:
...
Less interesting are the unit tests, these are mainly of interest for developing Olaf. The unit test can be compiled with `make test` and run with `./bin/olaf_tests`.

In the @eval@ folder there is an evaluation script which takes a folder as input and stores and evaluates queries with several modifications. SoX needs to be available on the system for this to work.

<pre>
ruby eval/olaf_evaluation.rb /folder/with/music
</pre>

Also present is a benchmark script which checks the performance of Olaf with larger datasets. With the script a folder of audio files is stored and it is registered how long it takes to store 64, 128, 256, 512,... files. If run with the "FMA full":https://github.com/mdeff/fma dataset a total of more than 200 days of audio are stored at a rate of just under 2000 times realtime with a 96 CPU-core system. An interpretation of the graph is that indexing remains linear on larger datasets. At every doubling of the database the "query performance":./eval/olaf_benchmark/olaf_benchmark_query.svg is also checked. Run the benchmark yourself:
