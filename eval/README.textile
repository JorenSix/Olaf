...
<small>Fig. Memory use of running a 20 second query with Olaf for several sizes of the index. On traditional computers a memory use of a few MB is negligible. This changes for embedded systems. The memory use of Olaf in that case is lower than 512kB value since the index is smaller, the queries are shorter and no key-value store is used.</small>
</div>

To detect memory leaks the macOS @leaks@ utility can be used in the following way:
...
h3(#eval). Evaluating Olaf
 ...
 True/false negative/positives are reported. There are a few options to make the evaluation more thorough or fit your use case. The number of tracks stored, the number of distractor, the number of true negatives, the severity of the modifications, the durations of the queries, ... can be configured. To run the evaluation:

<pre>
ruby eval/olaf_evaluation.rb /folder/with/music
</pre>

The result of an evaluation depends slightly on the input folder but should be similar to the following:
